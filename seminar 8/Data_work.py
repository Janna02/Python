...
def save_data():
    json.dump(student_data, open('\Educ\Python\seminar 8\student_data.json', 'w'))


def load_data():
    global student_data
    if os.stat('\Educ\Python\seminar 8\student_data.json').st_size == 0:
        student_data = {}
    else:
        student_data = json.load(open('\Educ\Python\seminar 8\student_data.json'))
